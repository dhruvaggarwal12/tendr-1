...
const PERFORMER_SERVICE_TYPES = new Set(['Singer', 'Band', 'Anchor', 'Choreographer', 'Musician', 'Emcee']);
const vendorPath = (svc, fromPlan) =>
  PERFORMER_SERVICE_TYPES.has(svc)
    ? `/artists/${encodeURIComponent(svc)}`
    : `/listings?serviceType=${encodeURIComponent(svc)}${fromPlan ? '&fromPlan=1' : ''}`;
              ...
              {selectedVendors.map(svc => {
                ...
                return (
                  <div key={svc} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                    ...
                    <button
                      onClick={() => navigate(vendorPath(svc, bookingType === 'you-do-it'))}
            ...
            <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", zIndex: 401, background: "#FFFCF5", borderRadius: 20, width: "min(95vw,440px)", boxShadow: "0 24px 60px rgba(0,0,0,0.25)", fontFamily: font, overflow: "hidden" }}>
              ...
              <form onSubmit={(e) => {
                ...
                <div>
                  ...
                  <input required type="date" value={savedMiniForm.date} min={(() => { const d = new Date(); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`; })()}
                    onChange={e => { const d = new Date(); const t = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`; if (e.target.value && e.target.value < t) return; setSavedMiniForm(p => ({ ...p, date: e.target.value })); }}
                  ...
                  {selectedVendors.map(svc => {
                    ...
                    return (
                      <div key={svc} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <button onClick={() => { navigate(vendorPath(svc, bookingType === 'you-do-it')); close(); }}
