...
// ── Standalone Invoice PDF (Tools tab) ───────────────────────────────────────
function generateStandaloneInvoicePDF(inv, vendorName) {
  const items = (inv.items||[]).filter(i => i.desc || Number(i.rate));
  const subtotal = items.reduce((s,i) => s + (Number(i.qty)||1)*(Number(i.rate)||0), 0);
  const discount = Number(inv.discount) || 0;
  const total = subtotal - discount;
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Invoice — ${escapeHtml(inv.clientName)}</title>
<style>body{font-family:'Outfit',Arial,sans-serif;max-width:680px;margin:40px auto;padding:0 24px;color:#2C1A0E;background:#fff}.hdr{display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:20px;border-bottom:2px solid #C47A2E;margin-bottom:24px}.brand{font-size:22px;font-weight:800;color:#C47A2E}.brand-sub{font-size:12px;color:#9B7450;margin-top:2px}h3{font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#9B7450;margin:20px 0 8px;border-bottom:1px solid #f0e8dc;padding-bottom:4px}.row{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #f8f2ea;font-size:13px}.row.bold{font-weight:700;font-size:15px;border-bottom:2px solid #C47A2E}table{width:100%;border-collapse:collapse;font-size:13px}th{text-align:left;font-size:11px;font-weight:700;color:#9B7450;text-transform:uppercase;letter-spacing:.08em;padding:6px 0;border-bottom:1.5px solid #e8ddd0}td{padding:9px 0;border-bottom:1px solid #f8f2ea;vertical-align:top}.total-box{background:#FFFCF5;border:1.5px solid #C47A2E;border-radius:10px;padding:14px 18px;margin-top:16px}footer{margin-top:40px;font-size:11px;color:#c0a880;text-align:center;border-top:1px solid #f0e8dc;padding-top:16px}@media print{body{margin:20px}}</style></head><body>
<div class="hdr"><div><div class="brand">Tendr</div><div class="brand-sub">${escapeHtml(vendorName)}</div></div><div style="text-align:right"><div style="font-size:18px;font-weight:800">TAX INVOICE</div><div style="font-size:13px;color:#6B4A2A;margin-top:3px">No: INV-${Date.now().toString().slice(-6)}</div><div style="font-size:13px;color:#6B4A2A">Date: ${new Date().toLocaleDateString('en-IN',{day:'numeric',month:'long',year:'numeric'})}</div></div></div>
<h3>Bill To</h3><div style="font-size:14px;font-weight:700">${escapeHtml(inv.clientName)}</div>${inv.clientPhone?`<div style="font-size:13px;color:#9B7450">📞 ${escapeHtml(inv.clientPhone)}</div>`:''}${inv.clientEmail?`<div style="font-size:13px;color:#9B7450">✉ ${escapeHtml(inv.clientEmail)}</div>`:''}${(inv.eventType||inv.eventDate)?`<h3>Event</h3>${inv.eventType?`<div class="row"><span>Type</span><span>${escapeHtml(inv.eventType)}</span></div>`:''}${inv.eventDate?`<div class="row"><span>Date</span><span>${new Date(inv.eventDate).toLocaleDateString('en-IN',{day:'numeric',month:'long',year:'numeric'})}</span></div>`:''}`:''}
<h3>Services</h3><table><thead><tr><th>Description</th><th style="text-align:center;width:60px">Qty</th><th style="text-align:right;width:100px">Rate (₹)</th><th style="text-align:right;width:110px">Amount (₹)</th></tr></thead><tbody>${items.map(i=>`<tr><td>${escapeHtml(i.desc||'—')}</td><td style="text-align:center">${Number(i.qty)||1}</td><td style="text-align:right">${(Number(i.rate)||0).toLocaleString('en-IN')}</td><td style="text-align:right;font-weight:700">${((Number(i.qty)||1)*(Number(i.rate)||0)).toLocaleString('en-IN')}</td></tr>`).join('')}</tbody></table>
<div class="total-box"><div class="row"><span>Subtotal</span><span>₹${subtotal.toLocaleString('en-IN')}</span></div>${discount>0?`<div class="row" style="color:#DC2626"><span>Discount</span><span>−₹${discount.toLocaleString('en-IN')}</span></div>`:''}<div class="row bold"><span>Total</span><span style="color:#C47A2E">₹${total.toLocaleString('en-IN')}</span></div></div>
${inv.notes?`<h3>Notes</h3><div style="font-size:13px;color:#6B4A2A;line-height:1.6">${escapeHtml(inv.notes).replace(/\n/g,'<br/>')}</div>`:''}
<footer>Generated by Tendr · tendr.in · Event Planning Platform</footer></body></html>`;
  const win = window.open('', '_blank');
  if (!win) return;
  win.document.write(html);
  win.document.close();
...
function generateContractPDF(c, vendorName, serviceType) {
  const today = new Date().toLocaleDateString('en-IN',{day:'numeric',month:'long',year:'numeric'});
  const advance = Number(c.advance||0);
  const amount  = Number(c.amount||0);
  const balance = Math.max(0, amount - advance);
  const defaultTerms = [
    `1. PAYMENT: ${advance>0?`Advance of ₹${advance.toLocaleString('en-IN')} is due on signing. Remaining ₹${balance.toLocaleString('en-IN')} is due on or before the event date.`:'50% advance is required to confirm the booking. Balance is due on the event date before services begin.'}`,
    `2. CANCELLATION: Cancellation >15 days before event — 50% of advance refunded. Within 15 days — advance non-refundable.`,
    `3. RESCHEDULING: One free reschedule with ≥10 days notice, subject to availability. Subsequent reschedules: ₹500 fee per change.`,
    `4. SERVICES: As described in this agreement. Additional services on the day will be charged separately at mutually agreed rates.`,
    `5. SETUP ACCESS: Client to ensure venue access and power supply at least 2 hours before event start time.`,
    `6. LIABILITY: ${escapeHtml(vendorName)} is not liable for delays due to force majeure, power failures, or venue access issues beyond their control.`,
    `7. PORTFOLIO: Unless agreed otherwise in writing, ${escapeHtml(vendorName)} may use event media for portfolio and promotional purposes.`,
    `8. GOVERNING LAW: This agreement is governed by Indian law. Disputes to be resolved amicably first; jurisdiction: Delhi courts.`,
  ].join('\n\n');
  const terms = c.customTerms || defaultTerms;
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Contract — ${escapeHtml(c.clientName)}</title>
<style>body{font-family:'Outfit',Arial,sans-serif;max-width:700px;margin:40px auto;padding:0 32px;color:#2C1A0E;background:#fff}.title-block{text-align:center;padding-bottom:24px;border-bottom:2px solid #2C1A0E;margin-bottom:28px}.title{font-size:22px;font-weight:800;letter-spacing:.05em;text-transform:uppercase}.sub{font-size:14px;color:#9B7450;margin-top:4px}.party-grid{display:grid;grid-template-columns:1fr 1fr;gap:24px;background:#fffbf5;border-radius:12px;padding:18px;margin:20px 0}.party-label{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#9B7450;margin-bottom:6px}.party-name{font-size:15px;font-weight:800}.detail-row{display:flex;gap:24px;flex-wrap:wrap;margin-bottom:10px}.detail-item{flex:1;min-width:100px}.detail-label{font-size:10px;font-weight:700;color:#9B7450;text-transform:uppercase;letter-spacing:.04em;margin-bottom:2px}.detail-val{font-size:14px;font-weight:700}.amount-box{background:linear-gradient(135deg,#2C1A0E,#4A2810);border-radius:14px;padding:18px 22px;color:#fff;display:flex;justify-content:space-between;align-items:center;margin:18px 0}.section-title{font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#C47A2E;padding-bottom:6px;border-bottom:1.5px solid #f0e8dc;margin:22px 0 12px}.terms{font-size:13px;color:#3D2510;line-height:2;white-space:pre-wrap}.sig-grid{display:grid;grid-template-columns:1fr 1fr;gap:48px;margin-top:56px}.sig-line{border-top:1.5px solid #2C1A0E;padding-top:8px;font-size:13px;font-weight:700}.sig-sub{font-size:11px;color:#9B7450;margin-top:3px}footer{margin-top:40px;font-size:11px;color:#c0a880;text-align:center;border-top:1px solid #f0e8dc;padding-top:16px}@media print{body{margin:20px}}</style></head><body>
<div class="title-block"><div class="title">Service Agreement</div><div class="sub">Tendr · ${escapeHtml(serviceType||'Event Services')}</div><div style="font-size:12px;color:#9B7450;margin-top:4px">Date: ${today}</div></div>
<div class="party-grid"><div><div class="party-label">Service Provider</div><div class="party-name">${escapeHtml(vendorName)}</div><div style="font-size:13px;color:#6B3A1F;margin-top:2px">${escapeHtml(serviceType||'')} · via Tendr</div></div><div><div class="party-label">Client</div><div class="party-name">${escapeHtml(c.clientName)}</div>${c.clientPhone?`<div style="font-size:13px;color:#6B3A1F;margin-top:2px">📞 ${escapeHtml(c.clientPhone)}</div>`:''}</div></div>
<div class="section-title">Event Details</div><div class="detail-row"><div class="detail-item"><div class="detail-label">Event Type</div><div class="detail-val">${escapeHtml(c.eventType||'—')}</div></div><div class="detail-item"><div class="detail-label">Event Date</div><div class="detail-val">${c.eventDate?new Date(c.eventDate).toLocaleDateString('en-IN',{day:'numeric',month:'long',year:'numeric'}):'—'}</div></div><div class="detail-item"><div class="detail-label">Location</div><div class="detail-val">${escapeHtml(c.location||'—')}</div></div></div>
<div class="amount-box"><div><div style="font-size:10px;font-weight:700;color:rgba(255,255,255,0.5);text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px">Contract Value</div><div style="font-size:28px;font-weight:900">₹${amount.toLocaleString('en-IN')}</div></div>${advance>0?`<div style="text-align:right"><div style="font-size:10px;color:rgba(255,255,255,0.5);text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px">Advance Due</div><div style="font-size:20px;font-weight:800;color:#CCAB4A">₹${advance.toLocaleString('en-IN')}</div></div>`:''}</div>
<div class="section-title">Terms & Conditions</div><div class="terms">${escapeHtml(terms)}</div>
<div class="sig-grid"><div><div style="height:48px"></div><div class="sig-line">${escapeHtml(vendorName)}</div><div class="sig-sub">Service Provider · Signature & Date</div></div><div><div style="height:48px"></div><div class="sig-line">${escapeHtml(c.clientName)}</div><div class="sig-sub">Client · Signature & Date</div></div></div>
<footer>This agreement is binding once signed by both parties · Generated via Tendr · tendr.in</footer></body></html>`;
  const win = window.open('', '_blank');
  if (!win) return;
  win.document.write(html);
  win.document.close();
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:500, background:'rgba(28,9,0,0.55)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }} onClick={onClose}>
      <div style={{ width:'100%', maxWidth:520, background:cream, borderRadius:20, boxShadow:'0 24px 60px rgba(0,0,0,0.22)', fontFamily:font, maxHeight:'92vh', display:'flex', flexDirection:'column' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding:'12px 20px 16px', borderTop:'1px solid rgba(196,122,46,0.1)', display:'flex', gap:8, flexShrink:0, flexWrap:'wrap' }}>
          <button onClick={onClose} style={{ padding:'10px 16px', borderRadius:10, border:'1.5px solid rgba(196,122,46,0.25)', background:'transparent', color:'#9B7450', fontFamily:font, fontSize:13, fontWeight:600, cursor:'pointer' }}>Cancel</button>
          <button onClick={() => generateQuote(form, '')} disabled={!form.clientName.trim()}
            style={{ padding:'10px 14px', borderRadius:10, border:'1.5px solid rgba(124,58,237,0.3)', background:'rgba(124,58,237,0.08)', color:'#7C3AED', fontFamily:font, fontSize:13, fontWeight:700, cursor:form.clientName.trim()?'pointer':'default', opacity:form.clientName.trim()?1:0.5, display:'flex', alignItems:'center', gap:6 }}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
            PDF
          </button>
          <button onClick={() => onSave(form)} disabled={!form.clientName.trim()}
            style={{ flex:1, padding:'10px', borderRadius:10, border:'none', background:form.clientName.trim()?`linear-gradient(135deg,${gold},${goldLt})`:'rgba(196,122,46,0.2)', color:'#fff', fontFamily:font, fontSize:13.5, fontWeight:800, cursor:form.clientName.trim()?'pointer':'default', boxShadow:form.clientName.trim()?'0 3px 12px rgba(196,122,46,0.3)':'none' }}>
            Save Quote
          </button>
        </div>
      </div>
...
const [tab, setTab] = useState('home');
  ...
  // Business Tools tab
  const [toolSection, setToolSection] = useState('quotes');
  const [toolInvoice, setToolInvoice] = useState({ clientName:'', clientPhone:'', clientEmail:'', eventType:'', eventDate:'', items:[{desc:'',qty:1,rate:''}], discount:'', notes:'' });
  const [toolContract, setToolContract] = useState({ clientName:'', clientPhone:'', eventType:'', eventDate:'', location:'', amount:'', advance:'', customTerms:'' });
  ...
  useEffect(() => {
    ...
    if (tab === 'profile' && user?.setlist && !setlistDraft) setSetlistDraft(user.setlist || '');
    ...
    <div style={{ display:'flex', minHeight:'100vh', background:'#FAF7F2', fontFamily:font }}>
      ...
      <div style={{ flex:1, marginLeft:isMobile?0:sideW, display:'flex', flexDirection:'column', minHeight:'100vh' }}>
        ...
        <div style={{ flex:1, padding:isMobile?'16px 14px 88px':'24px 28px 48px' }}>
            ...
            <>
              ...
              <div style={{ marginBottom: 18 }}>
                ...
                <button onClick={() => setTab('tools')}
                  style={{ width:'100%', padding:'14px 18px', borderRadius:14, background:'#fff', border:'1.5px solid rgba(196,122,46,0.18)', display:'flex', alignItems:'center', gap:14, cursor:'pointer', fontFamily:font, boxShadow:'0 2px 8px rgba(28,10,4,0.04)', textAlign:'left' }}>
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:13.5, fontWeight:800, color:ink }}>Quotes · Invoices · Contracts</div>
                    <div style={{ fontSize:11.5, color:'#9B7450', marginTop:1 }}>Create PDFs, use templates, manage estimates</div>
                  </div>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#C47A2E" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
                </button>
              </div>
          ...
          {tab === 'profile' && (() => {
            ...
            return (
              <div>
                ...
                {/* Setlist / Script (performers only) */}
                {isArtist && (
                  <div style={{ background:'#fff', borderRadius:18, padding:'18px 20px', border:'1px solid rgba(196,122,46,0.12)', marginBottom:20 }}>
                    <div style={{ fontSize:14, fontWeight:800, color:ink, marginBottom:4 }}>
                      {['DJ'].includes(serviceType) ? 'Typical Set / Playlist Theme' : ['Emcee/Host','Anchor'].includes(serviceType) ? 'Script Outline / Typical Segments' : serviceType === 'Stand-up Comedian' ? 'Set Material / Topics' : serviceType === 'Choreographer' ? 'Routine Description' : serviceType === 'Magician' ? 'Show Description' : 'Setlist / Repertoire'}
                    </div>
                    <div style={{ fontSize:12, color:'#9B7450', marginBottom:12 }}>Shown on your public profile — helps event planners understand your act</div>
                    <textarea value={setlistDraft} onChange={e => setSetlistDraft(e.target.value)} rows={5}
                      placeholder={['Emcee/Host','Anchor'].includes(serviceType) ? 'e.g. Welcome address, Games, Couple Q&A, Award rounds, Closing…' : 'List songs, sets, or themes you perform — helps event planners book you faster'}
                    ...
                    <button
                      onClick={async () => {
                        try {
                          await fetch(`${BASE}/vendors/${vendorId}/gigpro`, { method:'PATCH', headers:authHeaders(token), body:JSON.stringify({ setlist: setlistDraft }) });
                          showToast('Setlist saved!');
                        } catch { showToast('Could not save', false); }
                      }}
                      style={{ marginTop:10, padding:'9px 20px', borderRadius:9, border:'none', background:`linear-gradient(135deg,${gold},${goldLt})`, color:'#fff', fontFamily:font, fontSize:13, fontWeight:700, cursor:'pointer' }}>
                      Save Setlist
                    </button>
                    <button onClick={() => setTab('gig-profile')}
                      style={{ marginTop:10, marginLeft:8, padding:'9px 16px', borderRadius:9, border:`1.5px solid ${gold}`, background:'transparent', color:gold, fontFamily:font, fontSize:13, fontWeight:700, cursor:'pointer' }}>
          ...
          {/* ── BUSINESS TOOLS ── */}
          {tab === 'tools' && (() => {
            const toolInp = { width:'100%', padding:'8px 11px', borderRadius:9, border:'1.5px solid rgba(196,122,46,0.22)', fontFamily:font, fontSize:13, color:ink, outline:'none', background:'#FFFCF5', boxSizing:'border-box' };
            const toolLbl = (t) => <label style={{ display:'block', fontSize:11, fontWeight:700, color:'#6B3A1F', marginBottom:3 }}>{t}</label>;
            const TOOL_PILLS = [
              { key:'quotes',   label:'Quote Builder',    icon:'M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8zM14 2v6h6M16 13H8M16 17H8M10 9H8', accent:'#7C3AED' },
              { key:'invoice',  label:'Invoice Maker',    icon:'M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8zM14 2v6h6M12 18v-6M9 15h6', accent:gold },
              { key:'contract', label:'Contract Generator', icon:'M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z', accent:'#0EA5E9' },
            ];

            // Service-type-specific line item templates for quick fill
            const TEMPLATES = {
              DJ: ['DJ Performance (4 hrs)', 'Sound System Setup', 'LED Dance Floor Lighting', 'Extra Hour Charge', 'MC Hosting Add-on'],
              Singer: ['Solo Performance (45 min)', 'Backing Track Session', 'Live Band Accompaniment', 'Soundcheck (30 min)', 'Custom Song Learning Fee'],
              Musician: ['Live Performance (1 hr)', 'Rehearsal Session', 'Custom Arrangement Fee', 'Equipment Hire Add-on', 'Travel & Logistics'],
              Band: ['Live Band Performance (2 hrs)', 'Soundcheck & Setup', 'Extra Set (30 min)', 'Custom Song Arrangement', 'Equipment Transport'],
              Choreographer: ['Group Choreography (12 sessions)', 'Private Coaching (per session)', 'Flash Mob Coordination', 'Costume Consultation', 'Show Rehearsal Day'],
              Anchor: ['Event Anchoring (4 hrs)', 'Bilingual Script Writing', 'Games & Activities Hosting', 'Custom Script Consultation', 'Travel & Logistics'],
              'Emcee/Host': ['Event Hosting (4 hrs)', 'Custom Script Writing', 'Crowd Engagement Games', 'AV Coordination', 'Travel & Logistics'],
              Photographer: ['Wedding Photography (Full Day)', 'Pre-Wedding Shoot', 'Edited Photo Album (100 prints)', 'Digital Gallery Delivery', 'Second Photographer Add-on'],
              Videographer: ['Wedding Film (Full Day)', 'Cinematic Highlight Reel (5 min)', 'Raw Footage Delivery', 'Drone Add-on', 'Same-Day Edit'],
              Caterer: ['Veg Buffet (per plate)', 'Non-Veg Add-on (per plate)', 'Live Counters Setup (each)', 'Service Staff (per person per day)', 'Crockery & Chafing Dish Rental'],
              Decorator: ['Stage & Entrance Décor', 'Floral Centrepieces (per table)', 'Balloon Arch Setup', 'Lighting Arrangement', 'Cleanup & Breakdown'],
              'Makeup Artist': ['Bridal Makeup (full)', 'Bridesmaids Makeup (each)', 'Hair Styling', 'Pre-Wedding Trial', 'Touch-Up Kit'],
            };
            const tplList = TEMPLATES[serviceType] || ['Service Fee', 'Setup Charge', 'Materials Cost', 'Travel Allowance', 'Miscellaneous'];

            // Quote builder section
            const setInvItem = (i, k, v) => setToolInvoice(f => ({ ...f, items: f.items.map((it, idx) => idx === i ? { ...it, [k]: v } : it) }));
            const addInvItem = () => setToolInvoice(f => ({ ...f, items: [...f.items, {desc:'',qty:1,rate:''}] }));
            const removeInvItem = (i) => setToolInvoice(f => ({ ...f, items: f.items.filter((_,idx) => idx !== i) }));
            const invSubtotal = toolInvoice.items.reduce((s,i) => s+(Number(i.qty)||1)*(Number(i.rate)||0), 0);
            const invDiscount = Number(toolInvoice.discount)||0;
            const invTotal = invSubtotal - invDiscount;
            ...
            return (
              <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
                ...
                <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                  {TOOL_PILLS.map(p => (
                    <button key={p.key} onClick={() => setToolSection(p.key)}
                      style={{ display:'flex', alignItems:'center', gap:7, padding:'9px 18px', borderRadius:100, border:'1.5px solid', borderColor:toolSection===p.key?p.accent:'rgba(196,122,46,0.2)', background:toolSection===p.key?`${p.accent}18`:'#fff', color:toolSection===p.key?p.accent:'#6B3A1F', fontFamily:font, fontSize:13, fontWeight:700, cursor:'pointer', transition:'all 0.15s' }}>
                ...
                {toolSection === 'quotes' && (
                  <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
                    ...
                    <div style={{ background:'rgba(124,58,237,0.05)', borderRadius:14, padding:'14px 16px', border:'1.5px solid rgba(124,58,237,0.14)' }}>
                      <div style={{ fontSize:12, fontWeight:700, color:'#7C3AED', marginBottom:10 }}>⚡ Quick Templates — {serviceType || 'Common Services'}</div>
                      <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
                        {tplList.map((tpl, i) => (
                          <button key={i} onClick={() => { setEditQuote(null); setQuoteModal(true); }}
                            style={{ padding:'5px 12px', borderRadius:100, background:'rgba(124,58,237,0.1)', border:'1.5px solid rgba(124,58,237,0.22)', color:'#7C3AED', fontFamily:font, fontSize:12, fontWeight:600, cursor:'pointer' }}>
                            {tpl}
                          </button>
                        ))}
                      </div>
                      <div style={{ fontSize:11, color:'#9B7450', marginTop:8 }}>Click a template to open it in the quote builder, then edit and save.</div>
                    </div>

                    {/* New Quote button */}
                    <button onClick={() => { setEditQuote(null); setQuoteModal(true); }}
                      style={{ padding:'12px', borderRadius:11, border:'none', background:`linear-gradient(135deg,#7C3AED,#A855F7)`, color:'#fff', fontFamily:font, fontSize:14, fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
                    ...
                    {/* Saved quotes list */}
                    {quotes.length === 0 ? (
                      <div style={{ textAlign:'center', padding:'36px 24px', background:'#fff', borderRadius:16, border:'1px solid rgba(196,122,46,0.1)' }}>
                        <div style={{ fontSize:30, marginBottom:8 }}>📋</div>
                        <div style={{ fontSize:14, fontWeight:700, color:ink, marginBottom:4 }}>No quotes yet</div>
                        <div style={{ fontSize:12, color:'#9B7450' }}>Create your first estimate and download it as a PDF to share with clients.</div>
                      </div>
                    ) : (
                      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                        {quotes.map((q, qi) => {
                          const subtotal = (q.items||[]).reduce((s,i) => s+(Number(i.qty)||1)*(Number(i.rate)||0), 0);
                          const qTotal = subtotal - (Number(q.discount)||0);
                          return (
                            <div key={qi} style={{ background:'#fff', borderRadius:14, border:'1px solid rgba(196,122,46,0.12)', padding:'14px 16px', display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
                              <div style={{ flex:1, minWidth:0 }}>
                                <div style={{ fontSize:14, fontWeight:700, color:ink }}>{q.clientName || 'Client'}</div>
                                <div style={{ fontSize:11.5, color:'#9B7450', marginTop:2 }}>{q.eventType || ''}{q.eventDate ? ' · ' + new Date(q.eventDate).toLocaleDateString('en-IN',{day:'numeric',month:'short'}) : ''}</div>
                              </div>
                              <div style={{ fontSize:16, fontWeight:800, color:'#7C3AED' }}>₹{qTotal.toLocaleString('en-IN')}</div>
                              <div style={{ display:'flex', gap:8 }}>
                                <button onClick={() => { setEditQuote(q); setQuoteModal(true); }}
                                  style={{ padding:'6px 12px', borderRadius:8, border:'1.5px solid rgba(124,58,237,0.25)', background:'transparent', color:'#7C3AED', fontFamily:font, fontSize:12, fontWeight:700, cursor:'pointer' }}>Edit</button>
                                <button onClick={() => generateQuote(q, vendorName)}
                                  style={{ padding:'6px 12px', borderRadius:8, border:'none', background:'rgba(124,58,237,0.12)', color:'#7C3AED', fontFamily:font, fontSize:12, fontWeight:700, cursor:'pointer' }}>PDF ↗</button>
                              </div>
                            </div>
                          );
                        })}
                ...
                {toolSection === 'invoice' && (
                  <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                    {/* Templates */}
                    <div style={{ background:`rgba(196,122,46,0.06)`, borderRadius:14, padding:'14px 16px', border:`1.5px solid rgba(196,122,46,0.18)` }}>
                      <div style={{ fontSize:12, fontWeight:700, color:gold, marginBottom:10 }}>⚡ Quick Line Items — {serviceType || 'Common Services'}</div>
                      <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
                        {tplList.map((tpl, i) => (
                          <button key={i}
                            onClick={() => setToolInvoice(f => ({ ...f, items: f.items.some(it => it.desc === tpl) ? f.items : [...f.items.filter(it => it.desc), {desc:tpl,qty:1,rate:''}] }))}
                            style={{ padding:'5px 12px', borderRadius:100, background:`rgba(196,122,46,0.12)`, border:`1.5px solid rgba(196,122,46,0.25)`, color:'#7A5535', fontFamily:font, fontSize:12, fontWeight:600, cursor:'pointer' }}>
                            + {tpl}
                          </button>
                        ))}
                      </div>
                    ...
                    <div style={{ background:'#fff', borderRadius:16, padding:'18px 20px', border:'1px solid rgba(196,122,46,0.12)', display:'flex', flexDirection:'column', gap:12 }}>
                      <div style={{ display:'flex', gap:10 }}>
                        <div style={{ flex:1 }}>{toolLbl('Client Name *')}<input style={toolInp} value={toolInvoice.clientName} onChange={e => setToolInvoice(f=>({...f,clientName:e.target.value}))} placeholder="e.g. Rahul Sharma" /></div>
                        <div style={{ flex:1 }}>{toolLbl('Phone')}<input style={toolInp} type="tel" value={toolInvoice.clientPhone} onChange={e => setToolInvoice(f=>({...f,clientPhone:e.target.value}))} placeholder="10-digit" /></div>
                      </div>
                      <div>{toolLbl('Email (optional)')}<input style={toolInp} type="email" value={toolInvoice.clientEmail} onChange={e => setToolInvoice(f=>({...f,clientEmail:e.target.value}))} placeholder="client@email.com" /></div>
                      <div style={{ display:'flex', gap:10 }}>
                        <div style={{ flex:1 }}>{toolLbl('Event Type')}<select style={toolInp} value={toolInvoice.eventType} onChange={e => setToolInvoice(f=>({...f,eventType:e.target.value}))}><option value="">Select</option>{EVENT_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
                        <div style={{ flex:1 }}>{toolLbl('Event Date')}<input style={toolInp} type="date" value={toolInvoice.eventDate} onChange={e => setToolInvoice(f=>({...f,eventDate:e.target.value}))} /></div>
                      </div>

                      {/* Line items */}
                      <div>
                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
                          {toolLbl('Services / Line Items')}
                          <button onClick={addInvItem} style={{ padding:'4px 11px', borderRadius:7, border:'none', background:`linear-gradient(135deg,${gold},${goldLt})`, color:'#fff', fontSize:11.5, fontWeight:700, cursor:'pointer', fontFamily:font }}>+ Add</button>
                        </div>
                        <div style={{ border:'1.5px solid rgba(196,122,46,0.15)', borderRadius:10, overflow:'hidden' }}>
                          <div style={{ display:'grid', gridTemplateColumns:'1fr 52px 90px 26px', padding:'7px 10px', background:'rgba(196,122,46,0.05)', fontSize:10, fontWeight:700, color:'#9B7450', textTransform:'uppercase', letterSpacing:'0.08em' }}>
                            <span>Description</span><span style={{textAlign:'center'}}>Qty</span><span style={{textAlign:'right'}}>Rate (₹)</span><span/>
                          </div>
                          {toolInvoice.items.map((it, i) => (
                            <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr 52px 90px 26px', gap:4, padding:'7px 10px', borderTop:'1px solid rgba(196,122,46,0.08)', alignItems:'center' }}>
                              <input value={it.desc} onChange={e => setInvItem(i,'desc',e.target.value)} placeholder={`Item ${i+1}`} style={{ padding:'6px 8px', borderRadius:7, border:'1.5px solid rgba(196,122,46,0.2)', fontFamily:font, fontSize:12.5, color:ink, outline:'none', background:'#FFFCF5', width:'100%', boxSizing:'border-box' }} />
                              <input type="number" min="1" value={it.qty} onChange={e => setInvItem(i,'qty',e.target.value)} style={{ padding:'6px 4px', borderRadius:7, border:'1.5px solid rgba(196,122,46,0.2)', fontFamily:font, fontSize:12.5, color:ink, outline:'none', background:'#FFFCF5', width:'100%', boxSizing:'border-box', textAlign:'center' }} />
                              <input type="number" min="0" value={it.rate} onChange={e => setInvItem(i,'rate',e.target.value)} placeholder="0" style={{ padding:'6px 8px', borderRadius:7, border:'1.5px solid rgba(196,122,46,0.2)', fontFamily:font, fontSize:12.5, color:ink, outline:'none', background:'#FFFCF5', width:'100%', boxSizing:'border-box', textAlign:'right' }} />
                              <button onClick={() => removeInvItem(i)} disabled={toolInvoice.items.length===1} style={{ width:22, height:22, borderRadius:5, border:'1px solid rgba(220,38,38,0.2)', background:'transparent', color:'#DC2626', cursor:toolInvoice.items.length===1?'default':'pointer', fontSize:13, display:'flex', alignItems:'center', justifyContent:'center', opacity:toolInvoice.items.length===1?0.3:1 }}>×</button>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div style={{ display:'flex', gap:10, alignItems:'flex-end' }}>
                        <div style={{ flex:1 }}>{toolLbl('Discount (₹)')}<input style={toolInp} type="number" min="0" value={toolInvoice.discount} onChange={e => setToolInvoice(f=>({...f,discount:e.target.value}))} placeholder="0" /></div>
                        <div style={{ flex:1, padding:'9px 14px', borderRadius:9, background:'rgba(196,122,46,0.07)', border:'1px solid rgba(196,122,46,0.15)', textAlign:'right' }}>
                          <div style={{ fontSize:10, color:'#9B7450', textTransform:'uppercase', letterSpacing:'0.08em' }}>Total</div>
                          <div style={{ fontSize:20, fontWeight:800, color:gold }}>₹{invTotal.toLocaleString('en-IN')}</div>
                        </div>
                      </div>
                      <div>{toolLbl('Notes (payment terms, inclusions…)')}<textarea style={{ ...toolInp, height:60, resize:'vertical' }} value={toolInvoice.notes} onChange={e => setToolInvoice(f=>({...f,notes:e.target.value}))} placeholder="e.g. 50% advance to confirm booking. Balance due before event." /></div>

                      <button disabled={!toolInvoice.clientName.trim()} onClick={() => generateStandaloneInvoicePDF(toolInvoice, vendorName)}
                        style={{ padding:'12px', borderRadius:11, border:'none', background:toolInvoice.clientName.trim()?`linear-gradient(135deg,${gold},${goldLt})`:'rgba(196,122,46,0.2)', color:'#fff', fontFamily:font, fontSize:14, fontWeight:700, cursor:toolInvoice.clientName.trim()?'pointer':'default', display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
                ...
                {toolSection === 'contract' && (
                  <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                    {/* Pre-written terms preview */}
                    <div style={{ background:'rgba(14,165,233,0.05)', borderRadius:14, padding:'14px 16px', border:'1.5px solid rgba(14,165,233,0.18)' }}>
                      <div style={{ fontSize:12, fontWeight:700, color:'#0284C7', marginBottom:8 }}>📄 Pre-written Terms Included</div>
                      <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                        {['Payment & advance schedule','Cancellation & refund policy','One free reschedule clause','Equipment & setup responsibilities','Portfolio usage rights','Force majeure protection','Dispute resolution clause'].map((t,i) => (
                          <div key={i} style={{ display:'flex', alignItems:'center', gap:7, fontSize:12, color:'#0369A1' }}>
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                            {t}
                          </div>
                        ))}
                      </div>
                      <div style={{ fontSize:11, color:'#9B7450', marginTop:8 }}>These are auto-included. Add custom terms below to override or extend.</div>
                    </div>

                    <div style={{ background:'#fff', borderRadius:16, padding:'18px 20px', border:'1px solid rgba(196,122,46,0.12)', display:'flex', flexDirection:'column', gap:12 }}>
                      <div style={{ display:'flex', gap:10 }}>
                        <div style={{ flex:1 }}>{toolLbl('Client Name *')}<input style={toolInp} value={toolContract.clientName} onChange={e => setToolContract(f=>({...f,clientName:e.target.value}))} placeholder="e.g. Priya Mehta" /></div>
                        <div style={{ flex:1 }}>{toolLbl('Client Phone')}<input style={toolInp} type="tel" value={toolContract.clientPhone} onChange={e => setToolContract(f=>({...f,clientPhone:e.target.value}))} placeholder="10-digit" /></div>
                      </div>
                      <div style={{ display:'flex', gap:10 }}>
                        <div style={{ flex:1 }}>{toolLbl('Event Type')}<select style={toolInp} value={toolContract.eventType} onChange={e => setToolContract(f=>({...f,eventType:e.target.value}))}><option value="">Select</option>{EVENT_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
                        <div style={{ flex:1 }}>{toolLbl('Event Date')}<input style={toolInp} type="date" value={toolContract.eventDate} onChange={e => setToolContract(f=>({...f,eventDate:e.target.value}))} /></div>
                      </div>
                      <div>{toolLbl('Event Location')}<input style={toolInp} value={toolContract.location} onChange={e => setToolContract(f=>({...f,location:e.target.value}))} placeholder="e.g. The Grand, Vasant Kunj, Delhi" /></div>
                      <div style={{ display:'flex', gap:10 }}>
                        <div style={{ flex:1 }}>{toolLbl('Total Amount (₹) *')}<input style={toolInp} type="number" min="0" value={toolContract.amount} onChange={e => setToolContract(f=>({...f,amount:e.target.value}))} placeholder="e.g. 25000" /></div>
                        <div style={{ flex:1 }}>{toolLbl('Advance Amount (₹)')}<input style={toolInp} type="number" min="0" value={toolContract.advance} onChange={e => setToolContract(f=>({...f,advance:e.target.value}))} placeholder="e.g. 10000" /></div>
                      </div>
                      <div>{toolLbl('Custom Terms (optional — leave blank to use defaults)')}<textarea style={{ ...toolInp, height:100, resize:'vertical' }} value={toolContract.customTerms} onChange={e => setToolContract(f=>({...f,customTerms:e.target.value}))} placeholder={'Write your own terms, or leave blank to use Tendr\'s standard template:\n• Payment schedule\n• Cancellation policy\n• Rescheduling clause\n…and more'} /></div>

                      <button disabled={!toolContract.clientName.trim() || !toolContract.amount} onClick={() => generateContractPDF(toolContract, vendorName, serviceType)}
                        style={{ padding:'12px', borderRadius:11, border:'none', background:(toolContract.clientName.trim()&&toolContract.amount)?'linear-gradient(135deg,#0284C7,#0EA5E9)':'rgba(14,165,233,0.2)', color:'#fff', fontFamily:font, fontSize:14, fontWeight:700, cursor:(toolContract.clientName.trim()&&toolContract.amount)?'pointer':'default', display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
