...
export default function CustomerDashboard() {
  ...
  const [editingEvent, setEditingEvent]           = useState(null); // ev._id being edited
  ...
  const OCC_HUB_MAP = {
    ...
    'Kitty Party':'/kitty-party-hub','Naming Ceremony':'/naming-ceremony-hub','Graduation':'/graduation-hub',
    'Office Party':'/office-party-hub','Wedding':'/wedding-hub',
  };
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "clamp(16px,4vw,40px) clamp(14px,3vw,32px) 80px" }}>
        ...
        {showInstallBanner && (
          <div className="dashboard-install-banner" style={{ display: "flex", alignItems: "center", gap: 10, background: "linear-gradient(135deg,#2C1A0E,#4A2810)", borderRadius: 12, padding: "10px 14px", marginBottom: 16, flexWrap: "wrap" }}>
            <span style={{ flexShrink: 0, color: "#CCAB4A", display: "flex", alignItems: "center" }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="5" y="2" width="14" height="20" rx="2"/><line x1="12" y1="18" x2="12.01" y2="18"/></svg></span>
        ...
        <div className="dashboard-profile-card dash-in-0" style={{ background: "#FFFCF5", borderRadius: 16, border: "1.5px solid rgba(196,122,46,0.15)", boxShadow: "0 2px 12px rgba(139,69,19,0.07)", padding: "16px 20px", marginBottom: 20, display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              {user?.phoneNumber && (
                <span style={{ fontSize: 12, color: "#9B7450", display: "inline-flex", alignItems: "center", gap: 4 }}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.56 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 9.91a16 16 0 0 0 6.29 6.29l.91-.91a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>{user.phoneNumber}</span>
              )}
              {user?.email && (
                <span style={{ fontSize: 12, color: "#9B7450", display: "inline-flex", alignItems: "center", gap: 4 }}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>{user.email}</span>
              )}
            </div>
          </div>
                      ...
                      <div style={{ position:"relative", marginTop:10 }}>
                        ...
                        {bookNowOpen === ev._id && (
                          <div style={{ position:"absolute", top:"calc(100% + 6px)", left:0, right:0, background:"#fff", borderRadius:12, boxShadow:"0 8px 28px rgba(44,26,14,0.18)", border:"1.5px solid rgba(196,122,46,0.18)", zIndex:50, overflow:"hidden" }}
                            ...
                            <button
                              onClick={() => { setBookNowOpen(null); navigate(hubRoute || "/occasions"); }}
                              ...
                              <span style={{ fontSize:16 }}>🔍</span>
                              <div>
                                <div>{hubRoute ? `Open ${ev.occasion} Hub` : "Browse Vendors"}</div>
                                <div style={{ fontSize:11, color:"#9B7450", fontWeight:400 }}>{hubRoute ? "Games, tools & planning" : "Browse all vendors"}</div>
                              </div>
                            </button>
                            <button
