...
// Compress uploaded image to JPEG ≤ 400px wide at 60% quality (~20–50 KB)
...
function PlanIdeas({ orgType, venueType, onBookServices }) {
  ...
  const tabStyle = (active) => ({
    ...
    color: active ? saffron : muted,
    background: "transparent",
    borderBottom: active ? `2px solid ${saffron}` : `2px solid transparent`,
    transition: "all 0.15s",
  });
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", borderBottom: `1px solid ${border}`, marginBottom: 18 }}>
        ...
        <button style={tabStyle(tab === "checklist")} onClick={() => setTab("checklist")}>✅ Checklist</button>
            ...
            <div key={i} style={{ display: "flex", gap: 12, paddingBottom: 16, position: "relative" }}>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: 20, flexShrink: 0 }}>
                <div style={{
                  ...
                  background: i === content.timeline.length - 1 ? green : saffron,
                  marginTop: 4,
                }} />
              ...
              <div style={{ paddingBottom: 4 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: saffron, textTransform: "uppercase", letterSpacing: "0.08em" }}>{item.when}</div>
                <div style={{ fontSize: 13, color: dark, marginTop: 2, lineHeight: 1.5 }}>{item.text}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {content.checklist.map((item, i) => (
            <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
              <div style={{
                width: 18, height: 18, borderRadius: 5, border: `1.5px solid ${border}`,
                background: "#F8F4EF", flexShrink: 0, marginTop: 1,
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 10, color: muted,
              }}>☐</div>
              <div style={{ fontSize: 13, color: dark, lineHeight: 1.5 }}>{item}</div>
            </div>
          ))}
        </div>
      )}
      ...
      <button style={{ ...primaryBtn, marginTop: 24 }} onClick={onBookServices}>
        Book Services Now 🇮🇳
      </button>
    </div>
  );
...
export default function IndependenceDayFlow({ onClose }) {
  ...
  // org type is step 0
  const [orgType, setOrgType]   = useState(""); // "hr" | "community"
  ...
  // final screen: null | "choice" | "planideas"
  const [finalScreen, setFinalScreen] = useState(null);

  const hasDecorator = services.includes("decorator");

  // total steps for progress bar: org(0) + services(1) + venue(2) + details(3) + [decor(4)] + choice(5/4)
  const totalSteps = hasDecorator ? 5 : 4;

  // Fetch photos from API when venue type is set and decorator is selected
  useEffect(() => {
  ...
  function sendToBaatKaro() {
    ...
    // Uploaded venue photo first (compressed JPEG base64 — small enough for DB)
    if (uploadPreview) {
      chatPhotos.push({ url: uploadPreview, name: "Your venue", priceRange: "" });
    }

    // Selected reference decoration styles
    selectedPhotos.forEach((p) => {
      chatPhotos.push({ url: p.url, name: p.title, priceRange: p.description });
    });
      ...
      <div style={overlay} onClick={(e) => e.target === e.currentTarget && onClose()}>
        <div style={modal}>
          ...
          <div style={body}>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              >
                ...
                <div style={{ fontSize: 12, color: muted, marginTop: 3, lineHeight: 1.5 }}>
                  View {orgType === "hr" ? "corporate" : "community"} event timeline, checklist & decoration ideas tailored for you
                </div>
              </button>
            ...
            <button style={{ ...outlineBtn, width: "100%", marginTop: 14 }} onClick={() => setFinalScreen(null)}>
              ← Back to details
            </button>
          </div>
        </div>
  ...
  function renderStep() {
    ...
    if (step === 0) return (
      <div>
        <p style={{ fontSize: 14, color: "#7A4B1E", margin: "0 0 18px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <button
            ...
            style={{
              ...
              border: orgType === "hr" ? `2px solid ${saffron}` : `2px solid ${border}`,
          ...
          <button
            ...
            style={{
              padding: "18px 20px", borderRadius: 14, cursor: "pointer", textAlign: "left",
              border: orgType === "community" ? `2px solid ${saffron}` : `2px solid ${border}`,
    ...
    if (step === 1) return (
      <div>
        <p style={{ fontSize: 14, color: "#7A4B1E", margin: "0 0 16px" }}>
    ...
    if (step === 3) return (
      <div>
        <p style={{ fontSize: 14, color: "#7A4B1E", margin: "0 0 16px" }}>Tell us about your event:</p>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: "#7A4B1E", display: "block", marginBottom: 4 }}>
              {orgType === "hr" ? "Company Name & Organiser Name *" : "Host Name *"}
            </label>
            <input style={inp} placeholder={orgType === "hr" ? "e.g. Acme Corp — Priya Sharma (HR)" : "e.g. Ramesh Gupta"} value={hostName} onChange={(e) => setHostName(e.target.value)} />
          </div>
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: "#7A4B1E", display: "block", marginBottom: 4 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(130px, 1fr))", gap: 10 }}>
            <div>
              ...
              <input style={inp} type="number" placeholder={orgType === "hr" ? "120" : "80"} value={guests} onChange={(e) => setGuests(e.target.value)} />
            </div>
            <div>
          ...
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: muted, display: "block", marginBottom: 4 }}>
              Budget <span style={{ fontWeight: 400, fontSize: 11 }}>(optional)</span>
            </label>
            <input style={inp} placeholder="e.g. 50000" type="number" value={budget} onChange={(e) => setBudget(e.target.value)} />
          </div>
        </div>
        <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
          <button style={outlineBtn} onClick={() => setStep(2)}>← Back</button>
          <button
            style={{ ...primaryBtn, marginTop: 0, flex: 2, opacity: (hostName && address && guests && datetime) ? 1 : 0.5 }}
            disabled={!(hostName && address && guests && datetime)}
    ...
    // Step 4: decor picker (only if decorator selected)
    if (step === 4 && hasDecorator) {
      // Mode picker
      if (!decorMode) return (
        <div>
          <p style={{ fontSize: 14, color: "#7A4B1E", margin: "0 0 6px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <button
              ...
              style={{ padding: "18px 20px", borderRadius: 14, border: `2px dashed ${saffron}`, background: "rgba(255,153,51,0.05)", cursor: "pointer", textAlign: "left" }}
            >
            ...
            <button
              ...
              style={{ padding: "18px 20px", borderRadius: 14, border: `2px solid ${border}`, background: white, cursor: "pointer", textAlign: "left" }}
            >
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
            ...
            <button style={{ ...primaryBtn, marginTop: 0, flex: 2, background: "#E8D8C4", color: muted }} onClick={() => setFinalScreen("choice")}>
              Skip & continue
            </button>
          </div>
        </div>
      ...
      if (decorMode === "upload" || decorMode === "upload_done") return (
        <div>
          {uploadPreview ? (
            <>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#7A4B1E", marginBottom: 8 }}>📷 Your venue:</div>
    ...
    <div style={overlay} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div style={modal}>
        <div style={hdr}>
          ...
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", opacity: 0.85, marginBottom: 3 }}>
            STEP {step + 1} OF {totalSteps}
          </div>
          <div style={{ fontSize: "1.15rem", fontWeight: 700, margin: 0 }}>{stepTitles[step]}</div>
          <div style={{ fontSize: 12, opacity: 0.8, marginTop: 3 }}>Independence Day Celebration</div>
