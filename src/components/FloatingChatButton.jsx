...
export default function FloatingChatButton({ hideOnRoutes = ["/chat", "/chats", "/login", "/signup", "/otp"] }) {
  ...
  return (
    <>
      ...
      {/* WhatsApp FAB — left of chat, shows when GH/Stationery cart has items (pre-confirmation) */}
      {((ghCartCount > 0 && !ghConfirmed) || (stCartCount > 0 && !stConfirmed)) && (() => {
        const buildMsg = () => {
          let msg = "Hi Tendr! I'd like to place an order:\n\n";
          if (ghCartCount > 0 && !ghConfirmed && ghCartItems.length > 0) {
            msg += "🎁 *Gift Hampers*\n";
            ghCartItems.forEach(item => {
              msg += `• ${item.name} × ${item.quantity} — ₹${item.subtotal.toLocaleString("en-IN")}\n`;
            });
            msg += "\n";
          }
          if (stCartCount > 0 && !stConfirmed && stCartSnapshot.length > 0) {
            msg += "💒 *Wedding Stationery*\n";
            stCartSnapshot.forEach(({ item, quantity }) => {
              const price = item.priceOnRequest ? "On request" : item.priceRange || (item.startingPrice ? `₹${(item.startingPrice * quantity).toLocaleString("en-IN")}` : "");
              msg += `• ${item.name} × ${quantity} ${item.unit || "pcs"}${price ? ` — ${price}` : ""}\n`;
            });
            if (stForm?.date) msg += `\nEvent Date: ${stForm.date}`;
            if (stForm?.name) msg += `\nName: ${stForm.name}`;
            if (stForm?.address) msg += `\nAddress: ${stForm.address}`;
            msg += "\n";
          }
        ...
        return (
          <a
            href={`https://wa.me/919211668427?text=${encodeURIComponent(buildMsg())}`}
            target="_blank"
      ...
      <style>{`
        ...
        /* WhatsApp FAB — same slot as addons-fab (never shows simultaneously) */
        .whatsapp-fab {
          position: fixed;
          right: 168px;
          bottom: calc(22px + env(safe-area-inset-bottom, 0px));
        ...
        @media (max-width: 767px) {
          ...
          /* WhatsApp FAB: same slot as addons-fab on mobile */
          .whatsapp-fab {
            bottom: calc(80px + env(safe-area-inset-bottom, 0px)) !important;
