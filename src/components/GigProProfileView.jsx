...
const H_ICONS = [
  ...
  <svg key="h1" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>,
...
function BgDecor({ serviceType }) {
  const isDance = serviceType === "Choreographer" || serviceType === "Dancer";
  return (
    <div style={{ position:"absolute", inset:0, overflow:"hidden", pointerEvents:"none", zIndex:0, userSelect:"none" }}>
      {/* Large microphone outline — top-right */}
      <svg style={{ position:"absolute", right:"-3%", top:"2%", opacity:0.055, transform:"rotate(18deg)" }}
        width="230" height="290" viewBox="0 0 80 110" fill="none"
        stroke={gold} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
        <rect x="24" y="2" width="32" height="58" rx="16"/>
        <path d="M8 46c0 21 13 37 32 37s32-16 32-37"/>
        <line x1="40" y1="83" x2="40" y2="105"/>
        <line x1="26" y1="105" x2="54" y2="105"/>
      </svg>

      {/* Spotlight concentric circles — bottom right */}
      <svg style={{ position:"absolute", right:"-12%", bottom:"4%", opacity:0.04 }}
        width="340" height="340" viewBox="0 0 340 340" fill="none" stroke={gold} strokeWidth="1.5">
        <circle cx="170" cy="170" r="160"/>
        <circle cx="170" cy="170" r="120"/>
        <circle cx="170" cy="170" r="80"/>
      </svg>

      {/* Music note — bottom left, only for non-dance */}
      {!isDance && (
        <svg style={{ position:"absolute", left:"-2%", bottom:"10%", opacity:0.05, transform:"rotate(-6deg)" }}
          width="130" height="160" viewBox="0 0 60 75" fill="none"
          stroke={gold} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
          <path d="M22 60V10l30-8v50"/>
          <circle cx="17" cy="60" r="8"/>
          <circle cx="47" cy="52" r="8"/>
        </svg>
      )}

      {/* Stars — scattered */}
      {[
        { pct: [12, 32], sz: 16, op: 0.10 },
        { pct: [72, 15], sz: 11, op: 0.08 },
        { pct: [38, 55], sz: 13, op: 0.07 },
        { pct: [60, 42], sz: 9,  op: 0.09 },
        { pct: [20, 68], sz: 10, op: 0.06 },
        { pct: [84, 62], sz: 14, op: 0.07 },
      ].map((s, i) => (
        <svg key={i}
          style={{ position:"absolute", left:`${s.pct[0]}%`, top:`${s.pct[1]}%`, opacity:s.op }}
          width={s.sz} height={s.sz} viewBox="0 0 24 24" fill={gold} stroke="none">
          <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>
        </svg>
      ))}
      ...
      {/* Thin horizontal accent line behind header */}
      <div style={{
...
export default function GigProProfileView({ vendor, reviews = [], onBook, onChat }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:cream, fontFamily:font }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,500;0,700;0,800;1,400;1,700;1,800&family=Jost:wght@400;500;600;700;800&display=swap');
        *{box-sizing:border-box}

        /* Pill-style segmented tab bar */
        .gp-tabbar-wrap{padding:0 44px;margin-top:16px;position:relative;z-index:1}
        .gp-tabbar{
        ...
        .gp-tab.on{
          ...
          color:#fff;
          box-shadow:0 3px 14px rgba(196,122,46,0.42);
        }
        .gp-tab:hover:not(.on){color:rgba(28,10,4,0.75);background:rgba(196,122,46,0.07)}

        .gp-content{padding:32px 44px 80px;position:relative;z-index:1}
        ...
        .gp-header-bg{
          ...
          background:linear-gradient(155deg,rgba(196,122,46,0.09) 0%,transparent 58%);
          pointer-events:none;z-index:0;
        }
        .gp-header-inner{position:relative;z-index:1}
        .gp-dt-ctas{display:flex;gap:10px;margin-top:26px}
        .gp-stats-strip{
          display:flex;gap:0;padding:22px 44px;
          border-top:1px solid rgba(196,122,46,0.1);
          border-bottom:1px solid rgba(196,122,46,0.1);
          position:relative;z-index:1;
        ...
        .gp-right{position:relative;overflow:hidden}
        ...
        @media(max-width:899px){
          ...
          .gp-tabbar-wrap{padding:0 16px!important}
      ...
      <div className="gp-layout">
        ...
        {/* ── Right: scrollable panel with decorative BG ── */}
        <div className="gp-right">
          ...
          <div className="gp-mobile-hero">
            ...
            <div style={{ padding:"0 20px 16px" }}>
              <h1 style={{ fontFamily:serif, fontSize:"2.4rem", fontWeight:800, color:ink, lineHeight:1.05, margin:"0 0 8px", letterSpacing:"-0.02em" }}>{vendor.name}</h1>
          ...
          <div className="gp-desktop-header">
            ...
            <div className="gp-header-inner">
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap", marginBottom:16 }}>
                ...
                <span style={{ fontSize:14.5, fontWeight:600, color:ink, opacity:0.7 }}>{vendor.serviceType}</span>
                {city && (
                  <>
                ...
                {rating > 0 && (
                  <>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:5 }}>
                      <Stars r={rating} sz={13}/>
                      <span style={{ fontSize:14.5, fontWeight:800, color:ink }}>{rating.toFixed(1)}</span>
              ...
              {vendor.bio && (
                <p style={{ fontSize:15.5, color:"#4A3020", lineHeight:1.9, maxWidth:520, margin:"0 0 6px", fontWeight:400 }}>
                  {vendor.bio.length > 220 ? vendor.bio.slice(0,220)+"…" : vendor.bio}
                </p>
              )}

              <div className="gp-dt-ctas">
                <button onClick={onBook} style={{ padding:"13px 34px", borderRadius:100, background:`linear-gradient(135deg,${gold},${goldLt})`, color:"#fff", border:"none", fontSize:14.5, fontWeight:700, cursor:"pointer", fontFamily:font, boxShadow:"0 4px 20px rgba(196,122,46,0.38)", letterSpacing:"0.02em" }}>
                  Book Now
                </button>
                <button onClick={onChat} style={{ padding:"13px 28px", borderRadius:100, background:"transparent", border:`1.5px solid rgba(196,122,46,0.3)`, color:ink, fontSize:14, fontWeight:600, cursor:"pointer", fontFamily:font }}>
                  Message
                </button>
                {vendor.phoneNumber && (
            ...
            <div className="gp-stats-strip">
              {stats.map((s, i) => (
                <React.Fragment key={i}>
                  {i > 0 && <div style={{ width:1, background:"rgba(196,122,46,0.15)", margin:"0 32px" }}/>}
                  <div>
                    <div style={{ fontFamily:serif, fontSize:"2rem", fontWeight:700, fontStyle:"italic", color:gold, lineHeight:1 }}>{s.val}</div>
                    <div style={{ fontSize:10.5, color:muted, marginTop:6, letterSpacing:"0.1em", textTransform:"uppercase", fontWeight:700 }}>{s.label}</div>
                  </div>
                </React.Fragment>
              ))}
            </div>
          ...
          <div className="gp-content">
            ...
            {tab === "Portfolio" && (
              allPhotos.length > 0 ? (
                ...
                <div style={{ textAlign:"center", padding:"80px 20px", color:muted }}>
                  <svg style={{ opacity:0.2, marginBottom:16 }} width="52" height="52" viewBox="0 0 24 24" fill="none" stroke={gold} strokeWidth="1.5" strokeLinecap="round">
            ...
            {tab === "About" && (
              <div style={{ display:"flex", flexDirection:"column", gap:32, maxWidth:580 }}>
                {vendor.bio && (
                  <p style={{ fontSize:15.5, color:"#4A3020", lineHeight:1.9, margin:0 }}>{vendor.bio}</p>
                )}
                {highlights.length > 0 && (
                  <div style={{ display:"flex", flexDirection:"column", gap:0 }}>
                    {highlights.filter(h => h.label).map((h, i) => (
                      <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:16, paddingBlock:18, borderBottom:i < highlights.length-1 ? "1px solid rgba(196,122,46,0.07)" : "none" }}>
                        <div style={{ width:44, height:44, borderRadius:12, background:"rgba(196,122,46,0.09)", display:"flex", alignItems:"center", justifyContent:"center", color:gold, flexShrink:0 }}>
                ...
                {(genres.length > 0 || instr.length > 0) && (
                  <div>
                    ...
                    <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                      {[...genres, ...instr].map(g => (
                        <span key={g} style={{ background:"#fff", border:"1px solid rgba(196,122,46,0.18)", borderRadius:100, padding:"6px 16px", fontSize:13, color:"#4A3020", fontWeight:500 }}>{g}</span>
                      ))}
                    </div>
                  </div>
                )}
                ...
                <div style={{ display:"flex", flexDirection:"column" }}>
                  {setlistArr.map((line, i) => (
                    <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:18, paddingBlock:16, borderBottom:i < setlistArr.length-1 ? "1px solid rgba(196,122,46,0.07)" : "none" }}>
                      ...
                      <span style={{ fontSize:15, color:"#4A3020", lineHeight:1.6, fontWeight:500 }}>{line}</span>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ display:"flex", flexDirection:"column" }}>
                  {reviews.map((r, i) => (
                    <div key={i} style={{ paddingBottom:24, marginBottom:24, borderBottom:i<reviews.length-1?"1px solid rgba(196,122,46,0.08)":"none" }}>
                      ...
                      <p style={{ fontSize:14.5, color:"#4A3020", lineHeight:1.85, margin:0 }}>{r.reviewText}</p>
                    </div>
                  ))}
                </div>
              </div>
