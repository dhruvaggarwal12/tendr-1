...
export default function CancellationPolicy() {
  ...
  return (
    <div style={{ fontFamily: font, background: "#FFFCF5", minHeight: "100vh" }}>
      ...
      <style>{`
        .cp-dot-bg { background-color: #FFFCF5; background-image: radial-gradient(circle, rgba(196,122,46,0.22) 1.5px, transparent 1.5px); background-size: 26px 26px; }
        @keyframes cp-in { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }
        .cp-in-0 { animation: cp-in 0.52s 0.05s cubic-bezier(.22,1,.36,1) both; }
        .cp-in-1 { animation: cp-in 0.52s 0.13s cubic-bezier(.22,1,.36,1) both; }
        .cp-in-2 { animation: cp-in 0.52s 0.21s cubic-bezier(.22,1,.36,1) both; }
        @media (prefers-reduced-motion: reduce) { .cp-in-0,.cp-in-1,.cp-in-2 { animation: none !important; opacity: 1 !important; } }
      `}</style>
      {/* Hero */}
      <div className="cp-dot-bg" style={{ padding: "80px 24px 64px", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative" }}>
          <span className="cp-in-0" style={{ display: "inline-block", fontSize: 11, fontWeight: 800, letterSpacing: "0.2em", textTransform: "uppercase", color: "#C47A2E", background: "rgba(196,122,46,0.1)", border: "1px solid rgba(196,122,46,0.22)", padding: "4px 14px", borderRadius: 100, marginBottom: 18 }}>
            Policies
          </span>
          <h1 className="cp-in-1" style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "clamp(2.2rem, 5vw, 3.4rem)", fontWeight: 300, color: "#2C1A0E", margin: "0 0 16px", lineHeight: 1.15 }}>
            Cancellation <em style={{ color: "#C47A2E", fontStyle: "italic" }}>Policy</em>
          </h1>
          <p className="cp-in-2" style={{ fontSize: 17, color: "#7A5535", maxWidth: 540, margin: "0 auto", lineHeight: 1.65 }}>
