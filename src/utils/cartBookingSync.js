...
// whenever the user has fun-activity or stationery cart items.
// Called on login/signup and on add-to-cart (if already logged in).
// Fire-and-forget — never throws.
export function syncCartToBackend(token) {
  if (!token) return;
  try {
  ...
  fetch(`${BASE_URL}/conversations/baat-karo`, {
    ...
    credentials: 'include',
    body: JSON.stringify({ serviceType: 'Stationery & Activities' }),
  }).catch(() => {});
}
